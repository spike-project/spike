...
def do_proc_F1_flip_modu(dinp, doutp, parameter, nproc=None):
    ...
    f.write(str(dir(parameter)))
    f.write(str(parameter.do_F1))
    f.write(str(parameter.do_F2))
    f.write('nproc' + str(nproc))
    f.write('mpiutil.MPI_size = ' + str(mpiutil.MPI_size))
    f.close()
    mail("lionel.chiron@gmail.com","debug processing2D urqrd.py","sent with python",'debug_process_urqrd.txt' )
