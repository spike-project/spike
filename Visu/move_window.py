...
class MOVE_WINDOW():                                                                                    # class regrouping methods about drag function
    ...
    def move_refreshC_line(self, dx, dy):
        ...
        '''
        if debug(self): print('in move_refreshC_line ')
    ...
    def move_refreshC(self, dx, dy):
        ...
        '''
        if debug(self): print("in move_refreshC ")
        zc = self.paramz.zoom_coord
        try :
    ...
    def move_zoom_win(self, poswin, llx , lly, urx, ury):
        ...
        '''
        pwx, pwy = float(poswin[0]), float(poswin[1])
        winx, winy = (urx-llx)/2, (ury-lly)/2
    ...
    def moverectC(self):
        ...
        Drags the zoom area with mouse
        '''
        if debug(self): print("in moverectC ")
    ...
    def moverect(self, poswin, winselect):
        ...
        '''
        if debug(self): print("in moverect ")
        llx, lly, urx, ury = self.gtools.param_zoom_right_order()
        if winselect == "D" : self.moverectD(poswin, llx , lly, urx, ury)
        if winselect == "C" : self.moverectC()
        if winselect == "Czoom" : self.move_zoom_win(poswin, llx , lly, urx, ury) # 