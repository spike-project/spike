...
#module_path_parent = os.path.dirname(module_path)
#sys.path.append(module_path)
#sys.path.append(module_path_parent) # path to mail
#print "added paths ", module_path, module_path_parent
...
'''
under Linux or MacOsX : mpirun -n nbproc python processing2-urqrd.py (configfile.mscf)
under Windows : mpiexec -n nbproc python processing2-urqrd.py (configfile.mscf)
'''
...
def comp_sizesb(d0,  zflist, largest = LARGESTDATA, sizemin = SIZEMIN, vignette=True):
    """
    apply pred_sizes to all zerofiling values in zflist, and return a list with unique values
    largest determines the largest dataset allowed
    sizemini determines the minimum size when downzerofilling
...
def _do_proc_F1_flip_modu(data):
    ...
    import math as m
    c0, c1, shift, size, do_urqrd, urqrd_rank = data
    ...
    pbuf = d.col(0).buffer
    if do_urqrd:
        pbuf = urQRd(pbuf,urqrd_rank) #integrated urQRd
...
def iterarg(dinp, rot, size, do_urqrd, urqrd_rank ):
    "an iterator used by the MPI set-up"
    for i in range(0, dinp.size2, 2):
        c0 = dinp.col(i)
        c1 = dinp.col(i+1)
        yield (c0, c1, rot, size, do_urqrd, urqrd_rank)
    ...
    if nproc==None:
        ...
        for i in xrange( scan ):
            ...
            if parameter.do_urqrd: 
                p.buffer = urQRd(p.buffer,parameter.urqrd_rank) #integrated urQRd
            p.rfft()    
            p.modulus()
            doutp.set_col(i,p)
            pbar.update(i+1)
        ...
        xarg = iterarg(dinp, rot, size, parameter.do_urqrd, parameter.urqrd_rank)
        pool = mp.Pool(nproc)
...
class Proc_Parameters(object):
    ...
    def load(self, cp):
        ...
        self.urqrd_rank = cp.getint( "processing", "urqrd_rank", str(self.urqrd_rank)) # do_urqrd
