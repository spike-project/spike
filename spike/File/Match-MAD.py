...
def locate_ExciteSweep(folder):
    """
        ...
        It should always be in a subfolder 
    """
    L = glob.glob(op.join(folder,"*","ExciteChirp"))
    if len(L)>1:
        raise Exception( "You have more than 1 ExciteChirp file in the %s folder, using the first one"%folder )
    elif len(L) == 0: 
        raise Exception( "You don't have any ExciteChirp file in the  %s folder, please double check the path"%folder )
    return L[0]
#-----------------------------------------
def Import_1D(inifolder, outfile = "", compress=False):
    ...
    try:
        ...
        raise Exception('%s does not seem to be a valid Match spectrum'%(inifolder,))
    try:  #CR for compatibility with Apex, Solarix format as there is no ExciteChirp file
    #    l,h = read_ExciteSweep(locate_ExciteSweep(folder))
        h = locate_ExciteSweep(folder)
    except:
...
#    lowfreq = float(params["EXC_low"])
    ...
    if os.path.isfile(os.path.join(folder, "fid_f")):
        fname = os.path.join(folder, "fid_f")
    elif os.path.isfile(os.path.join(folder, "ser_f")):
    ...
    #extract correct conversion data for 1w/2w data and positive/negative mode
    QPD = int(params["QPD_Enabled"])
    API = int(params["API_Polarity"])
    if QPD == 0 and API == 0:
        data.axis1.calibA = float(params["ML1_positive"])
        data.axis1.calibB = float(params["ML2_positive"])
        data.axis1.calibC = float(params["ML3_positive"])
        data.axis1.specwidth = float(params["SW_h_Narrowband_1w"])
        data.axis1.highmass = float(params["EXC_hi_1w"])
        data.axis1.offsetfreq = float(params["FR_low_1w"])

    if QPD == 0 and API == 1:
        data.axis1.calibA = float(params["ML1_negative"])
        data.axis1.calibB = float(params["ML2_negative"])
        data.axis1.calibC = float(params["ML3_negative"])
        data.axis1.specwidth = float(params["SW_h_Narrowband_1w"])
        data.axis1.highmass = float(params["EXC_hi_1w"])
        data.axis1.offsetfreq = float(params["FR_low_1w"])

    if QPD == 1 and API == 0:
        data.axis1.calibA = float(params["ML1_positive_2w"])
        data.axis1.calibB = float(params["ML2_positive_2w"])
        data.axis1.calibC = float(params["ML3_positive_2w"]) 
        data.axis1.specwidth = float(params["SW_h_Narrowband_2w"])
        data.axis1.highmass = float(params["EXC_hi_2w"])
        data.axis1.offsetfreq = float(params["FR_low_2w"])

    if QPD == 1 and API == 1:
        data.axis1.calibA = float(params["ML1_negative_2w"])
        data.axis1.calibB = float(params["ML2_negative_2w"])
        data.axis1.calibC = float(params["ML3_negative_2w"])
        data.axis1.specwidth = float(params["SW_h_Narrowband_2w"])
        data.axis1.highmass = float(params["EXC_hi_2w"])
        data.axis1.offsetfreq = float(params["FR_low_2w"])
...
#    sizeF1 = int(params["L_20"]) #CR Not working if L_20 higher than 16383 
    ...
    if os.path.isfile(os.path.join(folder,"ser_f")):
        fname = os.path.join(folder, "ser_f")
    else:
        raise Exception("You are dealing with 1D data, you should use Import_1D")
    try:  #CR for compatibility with Apex and Solarix format as there is no ExciteChirp file
    #    l,h = read_ExciteSweep(locate_ExciteSweep(folder))
        h = locate_ExciteSweep(folder)
    except:
        raise Exception("No file found")
    ...
    API = int(params["API_Polarity"])
    if QPD == 0 and API == 0:
        data.axis2.calibA = float(params["ML1_positive"])
        data.axis2.calibB = float(params["ML2_positive"])
        data.axis2.calibC = float(params["ML3_positive"])
        data.axis2.specwidth = float(params["SW_h_Narrowband_1w"])
        data.axis2.highmass = float(params["EXC_hi_1w"])
        data.axis2.offsetfreq = float(params["FR_low_1w"])

    if QPD == 0 and API == 1:
        data.axis2.calibA = float(params["ML1_negative"])
        data.axis2.calibB = float(params["ML2_negative"])
        data.axis2.calibC = float(params["ML3_negative"])
        data.axis2.specwidth = float(params["SW_h_Narrowband_1w"])
        data.axis2.highmass = float(params["EXC_hi_1w"])
        data.axis2.offsetfreq = float(params["FR_low_1w"])

    if QPD == 1 and API == 0:
        data.axis2.calibA = float(params["ML1_positive_2w"])
        data.axis2.calibB = float(params["ML2_positive_2w"])
        data.axis2.calibC = float(params["ML3_positive_2w"]) 
        data.axis2.specwidth = float(params["SW_h_Narrowband_2w"])
        data.axis2.highmass = float(params["EXC_hi_2w"])
        data.axis2.offsetfreq = float(params["FR_low_2w"])

    if QPD == 1 and API == 1:
        data.axis2.calibA = float(params["ML1_negative_2w"])
        data.axis2.calibB = float(params["ML2_negative_2w"])
        data.axis2.calibC = float(params["ML3_negative_2w"])
        data.axis2.specwidth = float(params["SW_h_Narrowband_2w"])
        data.axis2.highmass = float(params["EXC_hi_2w"])
        data.axis2.offsetfreq = float(params["FR_low_2w"])

    data.axis2.lowfreq = data.axis2.offsetfreq
    data.axis2.left_point = 0.0
    ...
    if QPD == 1:
        data.axis1.calibA = data.axis2.calibA/2
        data.axis1.calibB = -data.axis1.offsetfreq/2 + data.axis2.calibB/2
        data.axis1.calibC = 0.0
...
#    data.adapt_size()
    NarrowMode = True
    if NarrowMode:
        data.axis2.itype = 1         # then assume data is complex
    npkbuf = np.fromfile(fname, 'f4')
    npkbuf.resize(sizeF1, sizeF2)
    data.buffer[:] = npkbuf[:]
    # if (outfile):
    #     HF.close()
    if (outfile):
